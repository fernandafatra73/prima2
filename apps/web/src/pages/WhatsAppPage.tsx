  ...
  return (
    <div>
      <div className="page-heading">
        <h2 className="page-heading__title">WhatsApp</h2>
      </div>
      ...
      >
        <div>
          <h3 style={{ margin: '0 0 0.35rem', fontSize: '1.15rem' }}>Buka WhatsApp Web</h3>
          <p style={{ margin: 0, fontSize: '0.85rem', opacity: 0.9, maxWidth: '520px' }}>
            WhatsApp tidak mengizinkan halamannya ditampilkan langsung di dalam aplikasi lain (dibatasi
            dari pihak WhatsApp sendiri), jadi tombol ini membuka WhatsApp Web di tab baru — cukup
            scan QR sekali dan sesi akan tetap tersimpan di browser ini.
          </p>
        </div>
        <a
      ...
      <p style={{ margin: '0 0 1rem', color: '#64748b', fontSize: '0.85rem' }}>
        Diambil dari data Daftar Telpon. Klik untuk langsung membuka chat WhatsApp dengan kontak tersebut.
      </p>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '0.75rem' }}>
          {filtered.map((item) => {
            const waNumber = formatWhatsAppNumber(item.telpon);
            return (
              <div
                key={item.id}
                style={{
              ...
              >
                ...
                {waNumber ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    title={`Chat WhatsApp dengan ${item.nama}`}
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: '0.35rem',
